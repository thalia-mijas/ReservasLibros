"use strict";
class Book {
    constructor(titulo, autor, publicacion, genero, copias) {
        this.title = titulo;
        this.author = autor;
    ...
    deleteBook(bookId) {
        ...
        if (bookStorage) {
            const books = JSON.parse(bookStorage);
    ...
    newBook() {
        const urlBook = document.getElementById("new-book-form")
            .action;
        if (urlBook) {
            const pathsBook = urlBook.split("=");
            ...
            const myObj = new Book(valueTitle, valueAuthor, valueYear, valueGenre, valueAvailability);
            if (bookStorage) {
                ...
                booksList === null || booksList === void 0 ? void 0 : booksList.push(myObj);
